...
class _IcebreakerSuggestions(TypedDict):
...
class _PublicChatMetadata(TypedDict):
    is_public: bool
...
class _SnoozedMessagesMetadata(TypedDict):
    snoozed_messages_count: int
    last_snoozed_message_timestamp_ms: int
    show_snooze_pill: bool


class _Nudge(TypedDict):
    create_time: None
    nudge_type: int


class _NicknamesSettingItem(TypedDict):
    user_igid: str
    nickname_setting: str


class _ParticipantRequestsCount(TypedDict):
...
class _ThreadContextItemsItem(TypedDict):
    text: str
    type: int


class _LastSeenAtValue(TypedDict):
    item_id: str
    timestamp: str
    created_at: str
    shh_seen_state: _Snippet


class _PrimaryCta(TypedDict):
    type: str
    title: None


class _SmartSuggestion(TypedDict):
    type: str
    title: None
    body: None
    request_id: None
    source: str
    primary_cta: _PrimaryCta
    secondary_cta: _PrimaryCta
    additional_info: _Snippet


class _ItemType(TypedDict):
    text: int


class _BtvEnabledMap(TypedDict):
    instamadillo_cutover: int
    item_type: _ItemType
    ttlc: int
    proton: int
    instamadillo_eligible: bool


class _IgThreadCapabilities(TypedDict):
    capabilities_0: int
    capabilities_1: int


class _PaidPartnershipInfo(TypedDict):
...
class _PreviewUrlInfo(TypedDict):
    url: str
    fallback: _Fallback
...
class _CtaButtonsItem(TypedDict):
    title: str
    action_url: str
    cta_type: None
    xma_action: str
...
class _XmaReelMentionItem(TypedDict):
    ...
    preview_url_info: _PreviewUrlInfo
    ...
    cta_buttons: list[_CtaButtonsItem]
    is_quoted: bool
    is_borderless: None
    is_sharable: bool
    verified_type: int
...
class _XmaClipItem(TypedDict):
    ...
    playable_url_mime_type: None
    preview_url_info: _PreviewUrlInfo
    header_icon_url_info: _PreviewUrlInfo
    header_icons_url_info: list[_PreviewUrlInfo]
    playable_url_info: None
    favicon_url_info: None
    favicon_style: None
    favicons_url_info: None
    ...
    preview_extra_urls_info: None
    preview_layout_type: int
    serialized_content_ref: str


class _ExpiredPlaceholder(TypedDict):
    message: str
    title: str


class _TextPartsItem(TypedDict):
    text: str


class _ActionLog(TypedDict):
    description: str
    bold: list[Any]
    text_attributes: list[Any]
    text_parts: list[_TextPartsItem]
    is_reaction_log: bool


class _VideoCallEvent(TypedDict):
    action: str
    vc_id: int
...
class _LastPermanentItem(TypedDict):
    item_id: str
    message_id: str
    user_id: int
    timestamp: int
    ...
    is_superlative: bool
    paid_partnership_info: _PaidPartnershipInfo
    ...
    genai_params: _Snippet
    system_folder: int
    auxiliary_text: str
    xma_reel_mention: list[_XmaReelMentionItem]
    xma_clip: list[_XmaClipItem]
    text: str
    processed_business_suggestion: bool
    tq_seq_id: int
    expired_placeholder: _ExpiredPlaceholder
    message_item_type: str
    action_log: _ActionLog
    hide_in_thread: int
    video_call_event: _VideoCallEvent
...
class _ThreadsItem(TypedDict):
    thread_id: str
    icebreakers: None
    icebreaker_suggestions: _IcebreakerSuggestions
    snippet: _Snippet
    dismiss_inbox_nudge: None
    should_upsell_nudge: bool
    public_chat_metadata: _PublicChatMetadata
    ...
    snoozed_messages_metadata: _SnoozedMessagesMetadata
    is_verified_thread: bool
    last_mentioned_item_timestamp_us: None
    lightweight_intervention_appealable_entity_id: None
    nudge: _Nudge
    nicknames: list[Any]
    nicknames_setting: list[_NicknamesSettingItem]
    participant_requests_count: _ParticipantRequestsCount
    ...
    thread_title: str
    thread_label: int
    thread_languages: dict[str, Any]
    is_group: bool
    is_spam: bool
    spam: bool
    users: list[_UsersItem]
    ...
    theme: _Theme
    thread_context_items: list[_ThreadContextItemsItem]
    professional_metadata: None
    pending: bool
    pending_user_ids: list[Any]
    last_seen_at: dict[str, _LastSeenAtValue]
    smart_suggestion: _SmartSuggestion
    system_folder: int
    ...
    thread_type: str
    thread_subtype: int
    btv_enabled_map: _BtvEnabledMap
    ...
    pending_user_expiration_timestamps: _Snippet
    is_stale: bool
    ig_thread_capabilities: _IgThreadCapabilities
    oldest_cursor: str
    newest_cursor: str
    last_permanent_item: _LastPermanentItem
    items: list[_ItemsItem]


class _PrevCursor(TypedDict):
    cursor_thread_v2_id: int
    cursor_timestamp_seconds: int
    cursor_relevancy_score: int


class _Inbox(TypedDict):
    threads: list[_ThreadsItem]
    pinned_threads: list[Any]
    unseen_count: int
    unseen_count_ts: int
    prev_cursor: _PrevCursor
    has_older: bool
    oldest_cursor: str
    next_cursor: _PrevCursor
    blended_inbox_enabled: bool


class _Viewer(TypedDict):
    strong_id__: str
    pk: int
    id: str
    fbid_v2: int
